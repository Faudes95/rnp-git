...
export async function openExpediente(page: Page, consultaId: number): Promise<void> {
  ...
  await expect(page.getByText("Expediente", { exact: false })).toBeVisible();
}
...
export async function saveInpatientDailyNote(page: Page, consultaId: number, hospitalizacionId: number): Promise<void> {
  await page.goto(`/expediente/inpatient-captura?consulta_id=${consultaId}&hospitalizacion_id=${hospitalizacionId}`);
  await page.locator('#nota-diaria').scrollIntoViewIfNeeded();
  ...
  await page.locator('#dailyNoteCaptureForm button[type="submit"][value="save"]').click();
