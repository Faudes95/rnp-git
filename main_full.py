...
def _svc_reporte_bi_extracted():
    return importlib.import_module("app.services.reporte_bi_extracted")

if MINIMAL_JEFATURA_BOOT:
    plt = None
...
RESIDENT_PROFILE_PHOTOS_DIR = os.path.abspath(
    os.getenv("RESIDENT_PROFILE_PHOTOS_DIR", os.path.join(PATIENT_FILES_DIR, "resident_profiles"))
)
try:
...
try:
    MAX_RESIDENT_PROFILE_PHOTO_SIZE_MB = max(1, int(os.getenv("MAX_RESIDENT_PROFILE_PHOTO_SIZE_MB", "5")))
except Exception:
    MAX_RESIDENT_PROFILE_PHOTO_SIZE_MB = 5
...
ALLOWED_RESIDENT_PROFILE_PHOTO_EXTENSIONS = {
    ".png",
    ".jpg",
    ".jpeg",
    ".webp",
...
class ResidentProfileDB(SurgicalBase):
    __tablename__ = "resident_profiles"

    id = Column(Integer, primary_key=True, index=True)
    resident_code = Column(String(120), nullable=False, unique=True, index=True)
    nombre = Column(String(200), index=True)
    sexo = Column(String(40), index=True)
    universidad = Column(String(180))
    sede_r1 = Column(String(180))
    inscripcion_unam = Column(String(120))
    photo_storage_path = Column(Text)
    photo_original_name = Column(String(255))
    photo_mime_type = Column(String(120))
    photo_updated_at = Column(DateTime, index=True)
    created_at = Column(DateTime, default=utcnow, nullable=False, index=True)
    updated_at = Column(DateTime, default=utcnow, onupdate=datetime.utcnow, nullable=False, index=True)


class ResidentPublicationDB(SurgicalBase):
    __tablename__ = "resident_publications"

    id = Column(Integer, primary_key=True, index=True)
    resident_code = Column(String(120), nullable=False, index=True)
    titulo = Column(String(240), nullable=False, index=True)
    revista_o_medio = Column(String(180))
    anio = Column(Integer, index=True)
    estatus = Column(String(80), index=True)
    notas = Column(Text)
    created_at = Column(DateTime, default=utcnow, nullable=False, index=True)
    updated_at = Column(DateTime, default=utcnow, onupdate=datetime.utcnow, nullable=False, index=True)


class ResidentCongressDB(SurgicalBase):
    __tablename__ = "resident_congresses"

    id = Column(Integer, primary_key=True, index=True)
    resident_code = Column(String(120), nullable=False, index=True)
    nombre_evento = Column(String(220), nullable=False, index=True)
    sede = Column(String(180))
    fecha = Column(Date, index=True)
    rol = Column(String(120), index=True)
    notas = Column(Text)
    created_at = Column(DateTime, default=utcnow, nullable=False, index=True)
    updated_at = Column(DateTime, default=utcnow, onupdate=datetime.utcnow, nullable=False, index=True)


class ResidentAwardDB(SurgicalBase):
    __tablename__ = "resident_awards"

    id = Column(Integer, primary_key=True, index=True)
    resident_code = Column(String(120), nullable=False, index=True)
    nombre = Column(String(220), nullable=False, index=True)
    institucion = Column(String(180))
    fecha = Column(Date, index=True)
...
class ResidentExamTemplateDB(SurgicalBase):
    __tablename__ = "resident_exam_templates"

    id = Column(Integer, primary_key=True, index=True)
    title = Column(String(220), nullable=False, index=True)
    description = Column(Text)
    period_label = Column(String(120), index=True)
    created_by = Column(String(120))
    is_active = Column(Boolean, default=True, nullable=False, index=True)
    created_at = Column(DateTime, default=utcnow, nullable=False, index=True)
    updated_at = Column(DateTime, default=utcnow, onupdate=datetime.utcnow, nullable=False, index=True)
    questions = relationship("ResidentExamQuestionDB", back_populates="exam", cascade="all, delete-orphan")
    assignments = relationship("ResidentExamAssignmentDB", back_populates="exam", cascade="all, delete-orphan")


class ResidentExamQuestionDB(SurgicalBase):
    __tablename__ = "resident_exam_questions"

    id = Column(Integer, primary_key=True, index=True)
    exam_template_id = Column(Integer, ForeignKey("resident_exam_templates.id", ondelete="CASCADE"), nullable=False, index=True)
    prompt = Column(Text, nullable=False)
    sort_order = Column(Integer, nullable=False, default=1, index=True)
    explanation = Column(Text)
    created_at = Column(DateTime, default=utcnow, nullable=False, index=True)
    options = relationship("ResidentExamOptionDB", back_populates="question", cascade="all, delete-orphan")
    exam = relationship("ResidentExamTemplateDB", back_populates="questions")


class ResidentExamOptionDB(SurgicalBase):
    __tablename__ = "resident_exam_options"

    id = Column(Integer, primary_key=True, index=True)
    question_id = Column(Integer, ForeignKey("resident_exam_questions.id", ondelete="CASCADE"), nullable=False, index=True)
    option_text = Column(Text, nullable=False)
    sort_order = Column(Integer, nullable=False, default=1, index=True)
    is_correct = Column(Boolean, default=False, nullable=False, index=True)
    question = relationship("ResidentExamQuestionDB", back_populates="options")


class ResidentExamAssignmentDB(SurgicalBase):
    __tablename__ = "resident_exam_assignments"

    id = Column(Integer, primary_key=True, index=True)
    resident_code = Column(String(120), nullable=False, index=True)
    exam_template_id = Column(Integer, ForeignKey("resident_exam_templates.id", ondelete="CASCADE"), nullable=False, index=True)
    periodo_label = Column(String(120), index=True)
    disponible_desde = Column(Date, index=True)
    cierra_en = Column(Date, index=True)
    ...
    created_at = Column(DateTime, default=utcnow, nullable=False, index=True)
    updated_at = Column(DateTime, default=utcnow, onupdate=datetime.utcnow, nullable=False, index=True)
    exam = relationship("ResidentExamTemplateDB", back_populates="assignments")
    attempts = relationship("ResidentExamAttemptDB", back_populates="assignment", cascade="all, delete-orphan")


class ResidentExamAttemptDB(SurgicalBase):
    __tablename__ = "resident_exam_attempts"

    id = Column(Integer, primary_key=True, index=True)
    assignment_id = Column(Integer, ForeignKey("resident_exam_assignments.id", ondelete="CASCADE"), nullable=False, index=True)
    ...
    assignment = relationship("ResidentExamAssignmentDB", back_populates="attempts")
    answers = relationship("ResidentExamAnswerDB", back_populates="attempt", cascade="all, delete-orphan")


class ResidentExamAnswerDB(SurgicalBase):
    __tablename__ = "resident_exam_answers"

    id = Column(Integer, primary_key=True, index=True)
    attempt_id = Column(Integer, ForeignKey("resident_exam_attempts.id", ondelete="CASCADE"), nullable=False, index=True)
    question_id = Column(Integer, ForeignKey("resident_exam_questions.id", ondelete="CASCADE"), nullable=False, index=True)
    selected_option_id = Column(Integer, ForeignKey("resident_exam_options.id", ondelete="CASCADE"), nullable=False, index=True)
    is_correct = Column(Boolean, default=False, nullable=False, index=True)
    attempt = relationship("ResidentExamAttemptDB", back_populates="answers")


class ResidentCaseAssignmentDB(SurgicalBase):
    __tablename__ = "resident_case_assignments"

    id = Column(Integer, primary_key=True, index=True)
    resident_code = Column(String(120), nullable=False, index=True)
    consulta_id = Column(Integer, index=True)
    patient_snapshot = Column(String(240), nullable=False, index=True)
    objetivo = Column(Text, nullable=False)
    prioridad = Column(String(40), nullable=False, default="MEDIA", index=True)
    fecha_limite = Column(Date, index=True)
    estado = Column(String(40), nullable=False, default="PENDIENTE", index=True)
    notas = Column(Text)
    assigned_by = Column(String(120))
    created_at = Column(DateTime, default=utcnow, nullable=False, index=True)
...
class ResidentIncidenceDB(SurgicalBase):
    __tablename__ = "resident_incidences"

    id = Column(Integer, primary_key=True, index=True)
    resident_code = Column(String(120), nullable=False, index=True)
    consulta_id = Column(Integer, index=True)
    tipo = Column(String(120), nullable=False, index=True)
    severidad = Column(String(40), nullable=False, default="MODERADA", index=True)
    fecha_evento = Column(Date, nullable=False, index=True)
    estado = Column(String(40), nullable=False, default="ABIERTA", index=True)
...
define_jefatura_quirofano_models = runpy.run_path(
    os.path.join(os.path.dirname(__file__), "app", "core", "jefatura_quirofano_models.py")
...
def _run_startup_schema_bootstrap() -> None:
    if MINIMAL_JEFATURA_BOOT:
        # El perfil mínimo no debe bloquearse creando todo el esquema legacy clínico.
...
def _build_advanced_reporte_metrics(*args, **kwargs):
    return _svc_reporte_bi_extracted().build_advanced_reporte_metrics(*args, **kwargs)


def generar_reporte_bi(*args, **kwargs):
    return _svc_reporte_bi_extracted().build_reporte_bi(*args, **kwargs)
...
def _get_api_routers_lazy():
    return importlib.import_module("app.api").get_api_routers(profile=APP_BOOT_PROFILE)
...
if MINIMAL_JEFATURA_BOOT:
    startup_interconexion = lambda: None
    startup_ai_agents_bootstrap = lambda: None
else:
